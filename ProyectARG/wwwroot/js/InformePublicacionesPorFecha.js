    ...
    success: function (informePublicacionesPorFechaMostrar) {
      ...
      for (let fechaPublicacionString in agrupadoPorFecha) {
        ...
        for (let nombreUsuario in agrupadoPorFecha[fechaPublicacionString]) {
          ...
          for (let provinciaLocalidad in agrupadoPorFecha[
            fechaPublicacionString
          ][nombreUsuario]) {
            contenidoTabla += `
                        <tr>
                            <td colspan="2"></td>
                            <td colspan="6" class="text-start" style="max-width: 100px; text-transform: Uppercase;">${provinciaLocalidad}</td>
                        </tr>
                        `;

            // Recorremos las publicaciones del usuario en esa provincia-localidad
            $.each(
              agrupadoPorFecha[fechaPublicacionString][nombreUsuario][
              provinciaLocalidad
              ],
              function (index, fecha) {
                contenidoTabla += `
                    <tr style="text-transform: Uppercase">
                        <td colspan="3"></td>
                        <td class="text-start" style="max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${fecha.tituloString
                  }</td>                            <td class="text-end" style="max-width: 60px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${fecha.precioString
                  } ${fecha.moneda ? "U$D" : "AR$"}</td>
                        <td class="text-start" style="max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${fecha.tipoInmuebleString
                  } - ${fecha.tipoOperacionString}</td>
                        <td class="text-start" style="max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${fecha.direccionString
                  } - ${fecha.nroDireccionString
                  }</td>                                
                    </tr>
                `;
              }
      ...
      document.getElementById("tbody-publicacionesPorFecha").innerHTML =
        contenidoTabla;
    },
